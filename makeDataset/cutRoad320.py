...
# savePlace = '/home/hua/pys/segmentation_models.pytorch/data/water480_320p80/'
savePlace = '/home/hua/pys/segmentation_models.pytorch/data/triRoad320/'
...
def cropTo(baseID, read_fold):
    ...
    for filename in os.listdir(read_fold):
        if 'b.' in filename:
            mask = cv2.imread( os.path.join(read_fold, filename))
            assert mask is not None
            img = cv2.imread(os.path.join(read_fold, filename.replace('b.', '.')))
            assert img is not None

            for i in range(100):
                y = i * (h // 4) #  per 80 or 160
                for j in range(100):
                    x = j * (w // 4)  # per 80 or  160
